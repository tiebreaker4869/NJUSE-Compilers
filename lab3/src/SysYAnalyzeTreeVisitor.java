...
public class SysYAnalyzeTreeVisitor extends SysYParserBaseVisitor<Void> {
    public static Map<String, String> map;

    private static Map<Symbol, List<Pair<Integer, Integer>>> locationMap;
    ...
    static {
        map = new HashMap<>();
        locationMap = SymTableVisitor.locationMappings;
        String[] orangeGroup = {
                "CONST",
                "INT",
                "VOID",
    ...
    public SysYAnalyzeTreeVisitor(int lineNo, int columnNo, String newName) {
        this.lineNo = lineNo;
    ...
    public Void visitTerminal(TerminalNode node) {
        ...
        tokenText = checkSubstitute(node.getSymbol());
    ...
    private String checkSubstitute(Token token) {

        for (Symbol symbol : locationMap.keySet()) {
            List<Pair<Integer, Integer>> lst = locationMap.get(symbol);
            int found = 0;

            for (Pair<Integer, Integer> p : lst) {
                if (p.a == lineNo && p.b == columnNo) {
                    found += 1;
                }

                if (p.a == token.getLine() && p.b == token.getCharPositionInLine()) {
                    found += 1;
                }
            }
