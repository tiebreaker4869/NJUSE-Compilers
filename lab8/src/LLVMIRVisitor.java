...
public class LLVMIRVisitor extends SysYParserBaseVisitor<LLVMValueRef> {
    ...
    private Map<LLVMValueRef, Boolean> ptrMap = new HashMap<>();

    private Map<LLVMValueRef, LLVMTypeRef> arrayTypeMap = new HashMap<>();
    ...
    public LLVMValueRef visitFuncDef(SysYParser.FuncDefContext ctx) {
        ...
        for (int i = 0; i < paramsCount; i ++) {
            if (ctx.funcFParams().funcFParam().get(i).L_BRACKT() == null || ctx.funcFParams().funcFParam().get(i).L_BRACKT().isEmpty()) {
                paramsType.put(i, i32Type);
            } else {
                paramsType.put(i, LLVMPointerType(i32Type, 0));
            }
        ...
        for (int i = 0; i < paramsCount; i ++) {
            ...
            if (funcFParamContext.L_BRACKT() == null || funcFParamContext.L_BRACKT().isEmpty()) {
                paramType = i32Type;
            } else {
                paramType = LLVMPointerType(i32Type, 0);
            }
            ...
            if (funcFParamContext.L_BRACKT() != null) {
                ptrMap.put(varPointer, true);
            }
        }
    ...
    public LLVMValueRef visitLVal(SysYParser.LValContext ctx) {
        ...
        if (ctx.L_BRACKT() == null || ctx.L_BRACKT().isEmpty()) {
            ...
            if (arrayTypeMap.containsKey(var)) {
                LLVMValueRef ptr = LLVMBuildAlloca(builder, LLVMPointerType(i32Type, 0), "tmp_");

                LLVMValueRef[] indexPtrArray = new LLVMValueRef[2];

                indexPtrArray[0] = indexPtrArray[1] = zero;

                PointerPointer<LLVMValueRef> indexPointer = new PointerPointer<>(indexPtrArray);

                LLVMValueRef arrayPtr = LLVMBuildGEP(builder, var, indexPointer, 2, "tmp_");

                LLVMBuildStore(builder, arrayPtr, ptr);
        ...
        if (ptrMap.containsKey(var)) {
            LLVMValueRef indexValue = visit(ctx.exp(0));

            LLVMValueRef[] arrayPointer = new LLVMValueRef[1];

            arrayPointer[0] = indexValue;

            PointerPointer<LLVMValueRef> indexPointer = new PointerPointer<>(arrayPointer);

            LLVMValueRef ptr = LLVMBuildLoad(builder, var, "tmp_");

            elementPtr = LLVMBuildGEP(builder, ptr, indexPointer, 1, "GEP_tmp");
        } else {
            LLVMValueRef indexValue = visit(ctx.exp(0));
            ...
            elementPtr = LLVMBuildGEP(builder, var, indexPointer, 2, "GEP_tmp");
        }
    ...
    public LLVMValueRef visitVarDecl(SysYParser.VarDeclContext ctx) {
        for (SysYParser.VarDefContext varDefContext : ctx.varDef()) {
            ...
            if (varDefContext.L_BRACKT() != null && !varDefContext.L_BRACKT().isEmpty()) {
                arrayTypeMap.put(varPointer, varType);
            }
